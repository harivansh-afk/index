...
use super::types::{self, Level, doc_block, type_name, value_name};
...
pub fn render(interface: &ir::Interface) -> Result<String, EmitError> {
    ...
    if types::records_use_buffer(interface) {
...
/// Where the record being rendered sits: the interface it belongs to and its
/// own position in declaration order, which is what decides whether a
/// reference to another record has to defer through `z.lazy`.
struct Scope<'a> {
...
/// optional, matching the `name?: T | null` the `.d.ts` declares: napi
/// reads a missing property as `None` and writes `None` back as `null`.
...
/// for type and position for position: a field's bytes check for a `Buffer`
/// exactly where the `.d.ts` declares one, and a container's interior
/// checks for the array of numbers the `.d.ts` declares there.
fn zod_type(scope: &Scope<'_>, ty: &ir::Type, level: Level) -> Result<String, EmitError> {
    Ok(match ty {
        ...
        // `z.instanceof` is the honest check for a `Buffer`: the value that
        // crosses is a real Node `Buffer`, and nothing weaker (a length
        // check, a number array) would distinguish it from the
        // `Array<number>` a container's bytes cross as. `z.infer` reads it
        // back as `Buffer`, matching the `.d.ts`.
        ir::Type::Bytes { .. } if level.bytes_as_buffer() => "z.instanceof(Buffer)".to_owned(),
        ir::Type::Bytes { .. } => "z.array(z.number().int())".to_owned(),
        ...
        ir::Type::Map { key, value } => {
            if !matches!(**key, ir::Type::String { .. }) {
                return Err(types::integer_keyed_map());
            }
