...
use jj_lib::working_copy::SubmoduleCheckoutOutcome;
...
impl CommandHelper {
    ...
    /// The backends this command can load, including any an extension added.
    ///
    /// A submodule's repo is loaded with these too, so an extension backend is
    /// usable in a submodule exactly where it is usable in the superproject.
            ...
            Ok(op) => {
                ...
                .await?
                {
                    ...
                    | WorkingCopyFreshness::SiblingOperation => {
                        let options = checkout_options(
                            ui,
                            &repo,
                            locked_ws.locked_wc().old_tree(),
                            &desired_wc_commit.tree(),
                            &self.data.store_factories,
                        )?;
...
",
    ...
    async fn update_working_copy(
        ...
        let stats = update_working_copy(
            ...
            maybe_old_commit,
            new_commit,
            self.env.command.store_factories(),
        )
        .await?;
    ...
    fn print_updated_working_copy_stats(
        ...
        print_checkout_stats(ui, stats, new_commit, self.env().path_converter())?;
        if Some(new_commit) != maybe_old_commit
            && let Some(mut formatter) = ui.status_formatter()
...
/// How Git submodules (gitlinks) take part in a conflict. No merge tool can
/// resolve either case, because jj records only a submodule's commit id and
/// never its contents, so there is nothing for a merge tool to merge.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum SubmoduleConflictKind {
    /// Every side that exists is a Git submodule, so the path is a submodule
    /// whose recorded commit id differs between sides.
    AllSubmodules,
    /// The path is a Git submodule on some sides and a file, symlink or
    /// directory on others.
    Mixed,
}

/// Classifies `conflict` by whether Git submodules take part in it, returning
/// `None` if none do.
///
/// Deleted sides are ignored: a submodule that exists on some sides and is
/// absent on others is still a conflict between submodule commit ids, and it
/// is resolved the same way.
pub fn submodule_conflict_kind(conflict: &MergedTreeValue) -> Option<SubmoduleConflictKind> {
    let mut any_submodule = false;
    let mut any_other = false;
    for term in conflict.iter().flatten() {
        match term {
            TreeValue::GitSubmodule(_) => any_submodule = true,
            _ => any_other = true,
        }
    }
    match (any_submodule, any_other) {
        (true, false) => Some(SubmoduleConflictKind::AllSubmodules),
        (true, true) => Some(SubmoduleConflictKind::Mixed),
        (false, _) => None,
    }
}
...
pub fn print_conflicted_paths(
    ...
    for ((_, conflict), formatted_path) in std::iter::zip(conflicts, formatted_paths) {
        ...
        let submodule_kind = submodule_conflict_kind(&conflict);
        ...
        for term in itertools::chain(conflict.removes(), conflict.adds()).flatten() {
            seen_objects.insert(
                match term {
                    ...
                    // An all-submodule conflict is described as being *on* a
                    // submodule below, so naming it here too would read as if
                    // a submodule were merely one of the things involved.
                    TreeValue::GitSubmodule(_)
                        if submodule_kind == Some(SubmoduleConflictKind::AllSubmodules) =>
                    {
                        continue;
                    }
        ...
        write!(formatter, "{formatted_path} ")?;
        {
            ...
            // Distinguish a conflict *on* a submodule, where every side is a
            // submodule commit id, from one that merely involves a submodule
            // among files or directories. The two need different remedies.
            if submodule_kind == Some(SubmoduleConflictKind::AllSubmodules) {
                write!(formatter, " on ")?;
                print_pair(*formatter, &("a git submodule".to_owned(), "difficult"))?;
            }
...
Discard the conflicting changes with `jj restore --from {}`.",
    ...
    print_submodule_checkout_stats(ui, stats, path_converter)?;
    print_submodule_dir_errors(ui, stats, path_converter)?;
    Ok(())
}

/// Reports what the checkout did with each Git submodule.
///
/// The four outcomes are printed separately rather than as one "skipped" list,
/// because what the user has to do about them differs: a submodule that has
/// never been cloned needs cloning, one whose commit is missing needs fetching,
/// and one that was left alone needs the user to decide what to do with the
/// files already in its directory.
fn print_submodule_checkout_stats(
    ui: &Ui,
    stats: &CheckoutStats,
    path_converter: &RepoPathUiConverter,
) -> Result<(), std::io::Error> {
    ...
    let mut print_group = |heading: &str, paths: &[&RepoPathBuf]| -> Result<bool, std::io::Error> {
        if paths.is_empty() {
            return Ok(false);
        }
    ...
    let paths_with = |wanted: fn(&SubmoduleCheckoutOutcome) -> bool| {
        stats
            .submodules
            .iter()
    ...
    print_group(
        "Populated these Git submodules:",
        &paths_with(|outcome| matches!(outcome, SubmoduleCheckoutOutcome::Populated { .. })),
    )?;
    if print_group(
        "These Git submodules have not been cloned, so their directories are empty:",
        &paths_with(|outcome| *outcome == SubmoduleCheckoutOutcome::NotCloned),
    )? {
        writeln!(
            ui.hint_default(),
            "Clone them with `jj git submodule clone`."
        )?;
    }
    print_group(
        "These Git submodule commits have not been fetched, so their directories are empty:",
        &paths_with(|outcome| *outcome == SubmoduleCheckoutOutcome::CommitNotFetched),
    )?;
    print_group(
        "Left these Git submodules alone because their directories are not empty:",
        &paths_with(|outcome| *outcome == SubmoduleCheckoutOutcome::LeftAlone),
    )?;
    print_group(
        "Did not look up the contents of these Git submodules:",
        &paths_with(|outcome| *outcome == SubmoduleCheckoutOutcome::NotResolved),
    )?;
    Ok(())
}

fn print_submodule_dir_errors(
    ui: &Ui,
    stats: &CheckoutStats,
    path_converter: &RepoPathUiConverter,
) -> Result<(), std::io::Error> {
    if !stats.submodule_dir_errors.is_empty() {
        writeln!(
            ui.warning_default(),
            "Failed to create directories for these Git submodules:"
        )?;
        let mut formatter = ui.stderr_formatter();
        for (path, err) in &stats.submodule_dir_errors {
            let ui_path = path_converter.format_file_path(path);
            writeln!(formatter, "  {ui_path}: {err}")?;
        }
    }
    Ok(())
}
...
pub async fn update_working_copy(
    ...
    // The tree on disk, not the old commit's: they are the same unless another
    // process checked something out, and it is the tree on disk that says which
    // files jj put in the working copy's submodule directories.
    let disk_tree = match workspace.working_copy().tree() {
        Ok(tree) => tree.clone(),
        Err(_) => old_tree
            .clone()
            .unwrap_or_else(|| repo.store().empty_merged_tree()),
    };
    let options = checkout_options(ui, repo, &disk_tree, &new_commit.tree(), store_factories)?;
...
/// Resolves the gitlinks the checkout is about to act on.
///
/// A repository whose gitlinks cannot be resolved at all, for instance because
/// its `.gitmodules` does not parse, still gets checked out: the submodule
/// design says that where support is incomplete jj should fall back and warn
...
fn checkout_options(
    ...
    new_tree: &MergedTree,
    store_factories: &StoreFactories,
) -> Result<CheckoutOptions, CommandError> {
    #[cfg(feature = "git")]
    {
        let resolved = jj_lib::git_submodule::resolve_submodule_contents(
            old_tree,
            new_tree,
            repo.submodule_store().as_ref(),
            repo.loader().settings(),
            store_factories,
        )
        .block_on();
        match resolved {
            Ok(submodules) => return Ok(CheckoutOptions { submodules }),
            Err(err) => {
                writeln!(
                    ui.warning_default(),
                    "Failed to resolve Git submodules; leaving their directories empty: {err}"
                )?;
            }
        }
    }
    Ok(CheckoutOptions::default())
}
