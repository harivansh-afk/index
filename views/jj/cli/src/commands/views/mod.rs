...
fn get_views_config(
    ...
    let path = workspace_root.join(VIEW_MANIFEST_FILE_NAME);
    if path.exists() {
        let body = fs::read_to_string(&path).map_err(|err| {
            user_error_with_message(format!("Could not read {}", path.display()), err)
        })?;
        let manifest: ViewManifest = toml::from_str(&body).map_err(|err| {
            user_error_with_message(format!("Could not parse {}", path.display()), err)
        })?;
        return manifest
            .views
            .into_iter()
...
fn anchor(
    ...
    Ok((name, gix::ObjectId::from_bytes_or_panic(target.as_bytes())))
}
...
/// Checks every configured endpoint and anchor before any command reaches the
/// network or mutates repository state.
fn validate_views(
    git: &Git,
    repo: &gix::Repository,
    configured: &[ViewConfig],
...
fn validate_endpoints(git: &Git, configured: &[ViewConfig]) -> Result<(), CommandError> {
    for view in configured {
...
fn working_copy_range(
    ...
    Some((
        gix::ObjectId::from_bytes_or_panic(parent.as_bytes()),
        gix::ObjectId::from_bytes_or_panic(wc_id.as_bytes()),
    ))
}
    ...
    while let Some(tree) = trees.pop() {
        ...
        for entry in decoded.entries {
            match entry.mode.kind() {
                ...
                _ => {
                    destination.find_object(entry.oid).map_err(|err| {
                        format!("anchor tree references missing blob {}: {err}", entry.oid)
                    })?;
                }
            }
        }
...
impl Git {
    ...
    /// Writes `tip` to the remote's branch.
    ///
    ...
    fn push(
        ...
        command
            .args(["--", remote])
            .arg(format!("{tip}:refs/heads/{branch}"));
