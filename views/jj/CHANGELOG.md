...
* New `jj views fetch` brings each configured view's published repository in and
  lifts its new commits into this one, under the view's path prefix. Like
  `jj git fetch` it only fast-forwards: a view whose two sides have each grown
  commits the other does not have is reported rather than merged or rebased, and
  the working copy is never moved. `--dry-run` reports what would be lifted. The
  report names how many of the incoming commits touch files already edited here.

* New `jj views status` says where each configured view stands without moving
  anything: ahead, behind, diverged, and how many published commits are here but
  elided from the view. That last number is the one that cannot be worked out
  from `git rev-list`, which counts an elided commit as behind forever. A
  diverged view is reported rather than refused, so one run answers for every
  view. `--no-fetch` reports against the last fetch and touches no network.

* `jj views fetch` now brings a diverged view's commits in rather than refusing.
  A view whose two sides have each grown commits the other does not have is a
  state it reports, as `jj git fetch` does; the published commits are lifted
  beside this repository's history and the bookmark is left where it was, so
  `jj new <bookmark> <revision>` integrates them. There is deliberately no
  `jj views merge` or `jj views rebase`: jj has no `merge` command and it does
  have `rebase`, so either would be a second spelling of something jj already
  provides, in a surface whose job is only the translation between this
  repository and the published one. The report names the revision, and warns
  that rebasing it rewrites commits the published repository has handed out.

* New `jj views push` publishes path prefixes of this repository to repositories
  of their own, in one command per repository rather than one per subtree. Each
  entry of the new `views` config table names a `path`, the `remote` it is
  published to, and that repository's own `branch`. The derived history's hashes
  are the published repository's, so what is sent is a branch it can
  fast-forward. By default it pushes a new branch and prints a URL to open a
  pull request; writing a view's default branch takes `--branch` naming it and
  `--allow-default-branch` as well.

* New `jj fs mount [-r REVSET] MOUNTPOINT` serves a revision's tree as a read-only
  filesystem, so any tool that reads files can read a revision without a
  checkout. It uses FUSE on Linux and an in-process NFSv3 server over loopback
  on macOS, where there is no in-kernel FUSE; neither path needs root. A
  conflicted file appears as a regular file holding the same conflict markers jj
  writes into a working copy. See [the guide](docs/vfs.md).

* New `jj git submodule list` shows the submodules a revision's `.gitmodules`
  declares. New `jj git submodule status` compares that declaration against the
  submodule store and against the gitlinks in the revision's tree, and reports
  separately whether a submodule was never cloned, was never fetched, is missing
  a gitlink at this revision, is held by the store but no longer declared, or is
  a gitlink `.gitmodules` does not declare at all. Both accept `-T`/`--template`
  and are configurable with `templates.git_submodule_list` and
  `templates.git_submodule_status`.
  [#1754](https://github.com/jj-vcs/jj/issues/1754),
  [#1759](https://github.com/jj-vcs/jj/issues/1759)

* New `merge_point()` revset function which (similar to `fork_point`) finds the
  point where multiple branches merge.

* New hidden `jj git submodule clone` command clones the Git submodules declared
  in `.gitmodules` at the working-copy commit into the submodule store. It is
  the first piece of submodule support and is nowhere near complete: nothing is
  populated in the working copy, nested submodules are reported but not cloned,
  and an already-cloned submodule is skipped rather than updated.
  [#1755](https://github.com/jj-vcs/jj/issues/1755)
...
* Checking out a commit now populates the working copy with the contents of any
  Git submodule that has a repository in jj's submodule store, and removes them
  again when checking out a commit without the submodule. A submodule that has
  not been cloned, or whose commit has not been fetched, gets an empty directory
  and is reported as such rather than being an error. A submodule directory
  holding anything jj did not write is left exactly as it is, so a clone made
  with `git submodule update` is never overwritten and uncommitted work inside a
  submodule is never deleted.

  Submodules stay readonly: changes made inside a populated submodule directory
  are still not snapshotted, and `jj sparse set` does not populate a submodule a
  new pattern brings into range.

### Fixed bugs

* `jj views fetch` no longer lifts the same published commit on every run. A
  commit the view elides -- one that changed nothing under the prefix and
  something outside it, which an upstream merge often does -- was invisible to
  the derivation that decides what has arrived, so each fetch lifted it again as
  a new commit. Deriving now records what an elided commit would have been, so
  the second fetch of a remote that has not moved is a no-op and says so, naming
  the elided count that makes raw ancestry read as behind. ENG-11873.

* jj no longer points you at a merge tool for a conflict on a Git submodule. A
  conflict whose every side is a submodule is now described as a conflict "on a
  git submodule" rather than one "including" one, and `jj resolve` on such a
  path fails with an explanation and suggests `jj restore --from <revision>`
  instead of starting a tool that cannot read a submodule. A path that is a
  submodule on only some sides of the conflict is reported separately. Resolving
  a submodule conflict in place is still unsupported.
  [#7806](https://github.com/jj-vcs/jj/issues/7806)
...
* `jj diff --git` now describes a Git submodule as a gitlink (mode `160000`)
  with a `Subproject commit <id>` body, like Git does. It previously used
  `040000`, the mode of a directory, and emitted no body, so `git apply`
  rejected the patch.

* Git submodules left alone while updating the working copy are now reported as
  part of the command's output instead of being written straight to stderr, so
  the message respects `--quiet` and color. Failing to create a submodule
  directory is now reported as a warning.
## [0.43.0] - 2026-07-01
