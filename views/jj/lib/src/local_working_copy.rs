...
use crate::working_copy::SubmoduleCheckoutOutcome;
...
/// untracked or modified files. This is in line with Git's behavior. Call
/// [`TreeState::give_back_submodule_dir`] first to remove the files jj itself
/// populated it with.
...
/// Whether the directory `disk_path` has no entries at all.
fn dir_is_empty(disk_path: &Path) -> Result<bool, CheckoutError> {
    let mut entries = fs::read_dir(disk_path).map_err(|err| CheckoutError::Other {
        message: format!("Failed to read directory {}", disk_path.display()),
        err: err.into(),
    })?;
    match entries.next() {
        None => Ok(true),
        Some(Ok(_)) => Ok(false),
        Some(Err(err)) => Err(CheckoutError::Other {
            message: format!("Failed to read directory {}", disk_path.display()),
            err: err.into(),
        }),
    }
...
/// Lists every file and symlink under `disk_path`, deepest directory last.
///
/// Returns `Ok(false)` as soon as it meets an entry whose name no tree entry
/// could have, since that is already proof that something other than jj wrote
/// in here and the caller is going to give up anyway.
///
/// Symlinks are listed as files and never followed, so a symlink to a directory
/// outside the submodule cannot lead the walk out of it.
fn walk_submodule_dir(
    disk_path: &Path,
    repo_path: &RepoPath,
    files: &mut Vec<(RepoPathBuf, PathBuf)>,
    dirs: &mut Vec<PathBuf>,
) -> Result<bool, CheckoutError> {
    let to_error = |err: io::Error| CheckoutError::Other {
        message: format!("Failed to read directory {}", disk_path.display()),
        err: err.into(),
    };
    for entry in fs::read_dir(disk_path).map_err(to_error)? {
        let entry = entry.map_err(to_error)?;
        let Ok(name) = entry.file_name().into_string() else {
            return Ok(false);
        };
        let Ok(component) = RepoPathComponentBuf::new(name) else {
            return Ok(false);
        };
        let child_repo_path = repo_path.join(&component);
        let child_disk_path = entry.path();
        let metadata = child_disk_path
            .symlink_metadata()
            .map_err(|err| checkout_error_for_stat_error(err, &child_disk_path))?;
        if metadata.is_dir() {
            if !walk_submodule_dir(&child_disk_path, &child_repo_path, files, dirs)? {
                return Ok(false);
            }
            dirs.push(child_disk_path);
        } else {
            files.push((child_repo_path, child_disk_path));
        }
    }
    Ok(true)
}
...
impl TreeState {
    ...
    /// Writes every file `tree` names into the Git submodule directory
    /// `disk_path`, which must be empty. Returns how many were written.
    ///
    /// No file state is recorded for these files. The superproject records
    /// `FileType::GitSubmodule` for the submodule root and nothing at all for
    /// what is inside it, which is what makes the snapshot code skip the whole
    /// directory and keeps this a readonly (Phase 1) feature: the files are
    /// checked out, but a change to one of them can never reach the store.
    async fn populate_submodule_dir(
        &self,
        disk_path: &Path,
        tree: &MergedTree,
    ) -> Result<u32, CheckoutError> {
        let mut added_files = 0;
        // The blobs live in the submodule's own store, not the superproject's,
        // so every read here goes through the tree rather than through `self`.
        let store = tree.store();
        for (path, value) in tree.entries() {
            let value = value?;
            // The submodule directory takes the place of the working copy root,
            // so every ".git"/".jj" and symlink check the superproject gets for
            // free applies to the submodule's paths too.
            let Some(file_disk_path) = create_parent_dirs(disk_path, &path)? else {
                continue;
            };
            let materialized = materialize_tree_value(store, &path, value, tree.labels()).await?;
            match materialized {
                MaterializedTreeValue::Absent | MaterializedTreeValue::AccessDenied(_) => {}
                MaterializedTreeValue::File(file) => {
                    // No previous exec bit can exist: nothing under a submodule
                    // is in the file states this would read it from.
                    let exec_bit =
                        ExecBit::new_from_repo(file.executable, self.exec_policy, || None);
                    self.write_file(&file_disk_path, file.reader, exec_bit, true)
                        .await?;
                    added_files += 1;
                }
                MaterializedTreeValue::Symlink { id: _, target } => {
                    if self.symlink_support {
                        self.write_symlink(&file_disk_path, target)?;
                    } else {
                        self.write_file(&file_disk_path, target.as_bytes(), ExecBit(false), false)
                            .await?;
                    }
                    added_files += 1;
                }
                MaterializedTreeValue::GitSubmodule(_) => {
                    // A submodule of a submodule. Phase 1 does not recurse, so
                    // this gets the same empty directory an unresolved gitlink
                    // gets in the superproject.
                    match fs::create_dir(&file_disk_path) {
                        Ok(()) => {}
                        Err(err) if err.kind() == io::ErrorKind::AlreadyExists => {}
                        Err(err) => {
                            return Err(CheckoutError::Other {
                                message: format!(
                                    "Failed to create nested submodule directory {}",
                                    file_disk_path.display()
                                ),
                                err: err.into(),
                            });
                        }
                ...
                MaterializedTreeValue::FileConflict(file) => {
                    ...
                    self.write_conflict(&file_disk_path, &contents, exec_bit)
                        .await?;
                    added_files += 1;
                }
                MaterializedTreeValue::OtherConflict { id, labels } => {
                    let contents = id.describe(&labels);
                    self.write_conflict(&file_disk_path, contents.as_bytes(), ExecBit(false))
                        .await?;
                    added_files += 1;
                }
                MaterializedTreeValue::Tree(_) => {
                    panic!("unexpected tree entry in submodule tree at {path:?}");
                }
            }
        }
        Ok(added_files)
    ...
    /// Removes the files jj populated the Git submodule directory `disk_path`
    /// with, leaving the directory itself for the caller to remove.
    ///
    /// jj records no file state for anything inside a submodule, so `tree`, the
    /// tree the outgoing gitlink named, is the only record of what jj put
    /// there. Anything not in `tree`, or whose contents no longer match it, is
    /// the user's: a clone they made themselves, or work they have not
    /// committed. Finding one such file makes jj remove nothing at all, rather
    /// than leave a half-emptied directory behind.
    async fn give_back_submodule_dir(
        &self,
        disk_path: &Path,
        tree: &MergedTree,
    ) -> Result<(), CheckoutError> {
        // The directory need not be there: an interrupted checkout, or a user
        // who removed it by hand, leaves a gitlink with nothing on disk.
        if !disk_path.is_dir() {
            return Ok(());
        }
        let mut expected: HashMap<RepoPathBuf, MergedTreeValue> = HashMap::new();
        for (path, value) in tree.entries() {
            expected.insert(path, value?);
        }
        let mut files = Vec::new();
        let mut dirs = Vec::new();
        if !walk_submodule_dir(disk_path, RepoPath::root(), &mut files, &mut dirs)? {
            return Ok(());
        }
        for (path, file_disk_path) in &files {
            let Some(value) = expected.get(path) else {
                return Ok(());
            };
            if !self
                .disk_file_matches_tree_value(
                    file_disk_path,
                    path,
                    value.clone(),
                    tree.store(),
        ...
        for (_, file_disk_path) in &files {
            fs::remove_file(file_disk_path).map_err(|err| CheckoutError::Other {
                message: format!(
                    "Failed to remove submodule file {}",
                    file_disk_path.display()
                ),
                err: err.into(),
            })?;
        }
        // Deepest last, so every directory is empty by the time it is reached.
        for dir_disk_path in dirs.iter().rev() {
            fs::remove_dir(dir_disk_path).map_err(|err| CheckoutError::Other {
                message: format!(
                    "Failed to remove submodule directory {}",
                    dir_disk_path.display()
                ),
                err: err.into(),
            })?;
        }
    ...
    /// Whether the file at `disk_path` is exactly what writing `value` there
    /// would have produced.
    ///
    /// The comparison is against the on-disk form, EOL conversion included, so
    /// that a repository configured to check out CRLF does not read its own
    /// checkout as modified.
    async fn disk_file_matches_tree_value(
        &self,
        disk_path: &Path,
        path: &RepoPath,
        ...
        let materialized = materialize_tree_value(store, path, value, labels).await?;
        let expected = match materialized {
            MaterializedTreeValue::File(mut file) => {
                let contents = file.read_all(path).await?;
                self.convert_eol_for_update(&contents).await?
            }
            MaterializedTreeValue::Symlink { id: _, target } => {
                if self.symlink_support {
                    let Ok(disk_target) = fs::read_link(disk_path) else {
                        return Ok(false);
                    };
                    return Ok(disk_target == symlink_target_convert_to_disk(&target));
                }
                target.into_bytes()
            }
            // A conflict, an access-denied entry or a nested submodule is not
            // worth reproducing byte for byte just to decide whether to delete
            // it. Treating it as modified only ever leaves files behind.
            _ => return Ok(false),
        };
    ...
    /// The bytes `contents` is written to disk as.
    async fn convert_eol_for_update(&self, contents: &[u8]) -> Result<Vec<u8>, CheckoutError> {
    ...
    pub fn check_out(
        ...
        let stats = self
            .update(&old_tree, new_tree, self.sparse_matcher().as_ref(), options)
            .block_on()?;
        self.tree = new_tree.clone();
        Ok(stats)
    }
    ...
    pub fn set_sparse_patterns(
        ...
        // A sparse-pattern change has no caller-resolved submodule contents, so
        // a submodule that a new pattern brings into range gets an empty
        // directory and is reported as `NotResolved`. See CHANGELOG.
        let options = CheckoutOptions::default();
        let added_stats = self
            .update(&empty_tree, &tree, &added_matcher, &options)
            .block_on()?;
        let removed_stats = self
            .update(&tree, &empty_tree, &removed_matcher, &options)
            .block_on()?;
        ...
        // Only the "added" side can report submodules: removing a path never
        // materializes a submodule.
        Ok(CheckoutStats {
            updated_files: 0,
            added_files: added_stats.added_files,
            removed_files: removed_stats.removed_files,
            skipped_files: added_stats.skipped_files,
            submodules: added_stats.submodules,
            submodule_dir_errors: added_stats.submodule_dir_errors,
        })
    }
    ...
    async fn update(
        ...
        let mut process_diff_entry = async |path: RepoPathBuf,
            ...
            // The commit a gitlink names, if the path was a Git submodule
            // before this checkout. It is the only record of what jj wrote into
            // that directory, so removal needs it.
            let before_submodule_id = match before.as_normal() {
                Some(TreeValue::GitSubmodule(id)) => Some(id.clone()),
                _ => None,
            };
            ...
            let present_file_deleted = before.is_present()
                && if let Some(id) = &before_submodule_id {
                    if let Some(SubmoduleSource::Tree(tree)) = options.submodules.get(&path, id) {
                        self.give_back_submodule_dir(&disk_path, tree).await?;
                    }
            ...
            let file_state = match after {
                ...
                MaterializedTreeValue::GitSubmodule(id) => {
                    // Git behavior: a submodule always gets a directory in the
                    // working copy, even when there is nothing to put in it.
                    let created = match fs::create_dir(&disk_path) {
                        Ok(()) => true,
                        Err(err) if err.kind() == io::ErrorKind::AlreadyExists => false,
                        Err(err) => {
                            stats
                                .submodule_dir_errors
                                .push((path.clone(), err.to_string()));
                            changed_file_states.push((path, FileState::for_gitsubmodule()));
                            return Ok(());
                        }
                    };
                    let outcome = if !created && !dir_is_empty(&disk_path)? {
                        // Anything already in there is either the user's own
                        // clone of the submodule or work they have not
                        // committed, and writing over it would destroy it.
                        SubmoduleCheckoutOutcome::LeftAlone
                    } else {
                        match options.submodules.get(&path, &id) {
                            None => SubmoduleCheckoutOutcome::NotResolved,
                            Some(SubmoduleSource::NotCloned) => SubmoduleCheckoutOutcome::NotCloned,
                            Some(SubmoduleSource::CommitNotFetched) => {
                                SubmoduleCheckoutOutcome::CommitNotFetched
                            }
                            Some(SubmoduleSource::Tree(tree)) => {
                                let added_files =
                                    self.populate_submodule_dir(&disk_path, tree).await?;
                                SubmoduleCheckoutOutcome::Populated { added_files }
                            }
                        }
                    };
                    stats.submodules.push((path.clone(), outcome));
            ...
            if after.is_absent() {
                ...
                let file_type = match after.into_resolved() {
                    Ok(value) => match value.unwrap() {
                        ...
                        // Unlike update(), reset() has no stats to report the
                        // skipped submodule through, so the caller isn't told
                        // about it. It only touches the recorded state anyway,
                        // never the submodule's contents on disk.
                        TreeValue::GitSubmodule(_id) => FileType::GitSubmodule,
                        TreeValue::Tree(_id) => {
                            panic!("unexpected tree entry in diff at {path:?}");
                        }
                    },
