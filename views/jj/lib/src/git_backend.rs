...
impl Backend for GitBackend {
    ...
    /// Answers from the object header rather than the object.
    ///
    /// A loose object carries its length in the first few bytes of its zlib
    /// stream and a packed one in the pack entry header, so neither is
    /// inflated. That matters because a filesystem serving a jj tree has to
    /// report a size before anything reads the file, and the default
    /// implementation would read every byte to find out.
    async fn file_size(&self, _path: &RepoPath, id: &FileId) -> BackendResult<u64> {
        let locked_repo = self.lock_git_repo();
        let git_blob_id = validate_git_object_id(&locked_repo, id)?;
