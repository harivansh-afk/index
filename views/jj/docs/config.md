...
- `templates.git_submodule_list` for `jj git submodule list`
- `templates.git_submodule_status` for `jj git submodule status`
...
A path prefix of this repository can also be a repository of its own. Filtering
the history to that prefix produces commits whose hashes are exactly the
published repository's, so publishing one is an ordinary `git push` of an
ordinary branch -- the part that is not ordinary is knowing which prefix goes to
which URL. A checked-in `.jj-views.toml` at the workspace root records that,
and `jj views push` acts on it.

```toml
[views.ix]
path   = "ix"
remote = "git@github.com:indexable-inc/ix.git"
branch = "main"

upstream-remote = "https://github.com/upstream/ix.git"
upstream-branch = "main"

[views.ix.anchor]
source = "0123456789abcdef0123456789abcdef01234567"
view = "89abcdef0123456789abcdef0123456789abcdef"
root = false
```

The read-only upstream pair and the anchor are optional. The upstream fields
must appear together. `jj views fetch --upstream ix` and `jj views status
--upstream ix` select that endpoint. Push always uses `remote` and `branch`.

An anchor maps a commit in this repository to the published view commit with
the same filtered tree. Both ids are checked before use. The source must be an
ancestor of the revision being derived, and filtering its tree must equal the
published commit's tree. Derivation then reads only commits after the anchor.
Set `root = true` only when creating a branch whose remote does not already
hold the anchor's ancestry. `jj views anchor` then recreates `view` by filtering
`source` and removing its parents. The manifest's `view` id checks the generated
commit before it is written. This parentless snapshot lets the first push satisfy
the remote's object-connectivity check without importing history before the
anchor.
Run `jj views anchor ix` once when a fresh clone lacks the published anchor
object. A local object needs no network. Otherwise the command fetches the
advertised published branch. That fetch has depth equal to the linear host patch
count after `anchor.source`, plus the anchor itself. The anchor must be its exact
shallow boundary, and every fetched commit must equal the view derived from the
corresponding host commit. If the published branch is absent during initial
bootstrap, the command asks the read-only upstream for the exact anchor at depth
1. The command reports each failed endpoint attempt before naming the endpoint
that succeeded. It installs only the validated anchor object and no anchor
parents.

When `.jj-views.toml` is absent, the same fields remain supported in local
`[views.NAME]` configuration. A manifest takes precedence when present so a
fresh clone gets the repository's checked view definitions.

`branch` is the published repository's own default branch. It is the base a
pull request is opened against, and it is the one branch `jj views push` will
not write unless you name it with `--branch` *and* pass
`--allow-default-branch`. By default the command pushes a new branch named by
the `templates.git_push_bookmark` setting, the same name `jj git push --change`
would generate, and prints a URL to open a pull request from it. It prints that
URL whether the branch moved or was already there, since re-pushing a change
under review is the common case.

A view will not publish a tip commit with an empty description, the same refusal
`jj git push` makes and behind the same `--allow-empty-description` flag. The
description a view sends is the monorepo commit's, so an undescribed change here
becomes an undescribed commit in a repository other people read.

Every configured view is derived before any of them is pushed, so a bad prefix
or a refused branch fails before anything reaches a remote. Pushing then happens
in the table's key order and stops at the first failure, and the report names
which views landed, which failed, and which were never attempted.

`jj views push` does not push the repository you are in, and moves nothing in
it: its own bookmarks, tags and remotes are exactly as they were afterwards.
Sending those is `jj git push`, which has bookmark, tracking and lease semantics
of its own.

`jj views fetch` is the other direction, and follows `jj git fetch`: it brings
each published repository's branch in, turns the commits it has and this one
does not into commits under the prefix, and moves a bookmark to the result. It
only fast-forwards. If both sides have grown commits the other does not have it
stops and reports it, rather than picking a merge or a rebase for you. It does
not move the working copy.

It does bring the commits in either way. For a diverged view they are lifted
onto the counterpart of the commit the two sides last agreed on, so they land
*beside* this repository's history rather than on top of it, and the bookmark
stays where it was. `jj views fetch` then names the revision they arrived as,
and integrating them is `jj new <bookmark> <revision>`.

There is no `jj views merge` and no `jj views rebase`, on purpose. jj has no
`merge` command -- a merge is `jj new A B` -- and it does have `rebase`, so
either name would be a second spelling of something jj already provides, in a
surface whose job is only translating between this repository and the published
one. It also puts the one dangerous choice in a command whose consequences the
user already knows: rebasing the arrived lineage onto the local one re-derives
every published commit to a new hash, so the published repository ends up
holding the content without the commits, and neither side can see that until
somebody's fetch fails.

`jj views status` answers where each view is without moving anything, and is the
one to reach for when a view looks wrong. Alongside ahead, behind and diverged
it reports how many published commits are here and *elided* from the view.
`jj views status --json` emits one object with a nonempty `views` list. Each
entry names the path, selected endpoint, anchor ids, compared commits, counts,
anchor tree check, and one closed state: `up_to_date`, `local_ahead`,
`fast_forward`, or `diverged`.

That last count is worth understanding, because it is the one `git rev-list`
cannot give you. A commit that changed nothing under the prefix and something
outside it is dropped by the filter -- that rule is what keeps a view free of
the monorepo's unrelated churn, and is also what preserves the published
hashes. So a published merge that touched nothing under the prefix arrives here
and then vanishes from the derived history. `git rev-list <view tip>..<upstream>`
counts it forever, no fetch will ever remove it from that count, and "3 commits
behind" and "up to date, 3 commits elided" look identical without the number.

`jj views prompt` is the machine end of `status`, for shell prompts. Run
inside a view's path it prints one tab-separated line -- the view's name,
commits behind, commits ahead -- and outside every view it prints nothing.
The counts are read from a record the last `fetch` or `status` wrote rather
than computed, so with `--ignore-working-copy` it is cheap enough to run per
prompt, and exactly as fresh as the last survey.
