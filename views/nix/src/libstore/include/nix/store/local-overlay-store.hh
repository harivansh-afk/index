...
private:
    ...
    /**
     * Whether a store object the lower store calls valid can actually be
     * read through the overlay.
     *
     * The lower store is allowed to gain store objects while an overlay
     * over it is mounted, but the kernel does not promise to show them
     * there: a lookup that missed before the lower store gained the entry
     * leaves a negative dentry that is never revalidated, so the merged
     * directory keeps answering ENOENT for a store object that is sitting
     * in the lower layer. `remount-hook` is the documented way out and is
     * not always available -- the new mount API refuses to reconfigure an
     * overlay mount (see `tests/functional/local-overlay-store/common.sh`).
     *
     * So validity has to be answered about the layer this store actually
     * serves objects from, not only about the lower store's database.
     */
    bool lowerPathVisible(const StorePath & path);
    ...
    /**
     * State for `lowerPathVisible`'s one-shot warning.
     */
    std::atomic_bool _warnedLowerInvisible = false;
};
