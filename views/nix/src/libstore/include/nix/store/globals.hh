...
public:
    ...
    Setting<std::string> writeThroughStore{
        this,
        "",
        "write-through-store",
        R"(
          The [URL of a Nix store](@docroot@/store/types/index.md#store-url-format) that
          locally built outputs are published to as part of the build.

          When this is set to a store URL, every locally built derivation has its outputs
          copied to that store after the outputs are registered valid and before the build
          goal reports success, and a failure to copy fails the build. Publication is
          therefore synchronous with the build: once a build reports success its outputs
          are durable on the destination, which leaves the local store free to be treated
          as a prunable cache.

          Only local builds are published this way. Store objects obtained by substitution,
          and outputs of builds delegated to a remote builder, are already durable
          elsewhere.

          Failing the build is deliberate, and it is the reason this is a build step rather
          than a queue. The previous design deferred publication to a queue drained after
          the build; on 2026-08-02/03 that queue was measured at 65,996 entries against a
          65,536 bound, at which point it both blocked CI admission and pinned every queued
          path against garbage collection. A build whose outputs exist in exactly one place
          has not been made durable, so it should not report success.

          The default is the empty string, which disables write-through publication.
        )"};
