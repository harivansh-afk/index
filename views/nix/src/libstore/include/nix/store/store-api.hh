...
public:
    ...
    /**
     * Fingerprint for the path `pathInStore` (relative to `storeDir`)
     * as served by the whole-store accessors returned by
     * `getFSAccessor()`; see `SourceAccessor::getFingerprint()` for the
     * contract. Returns the path within the enclosing store object plus
     * that object's fingerprint, or no fingerprint when `pathInStore`
     * does not name a valid store object.
     */
    std::pair<CanonPath, std::optional<std::string>> getStoreObjectFingerprint(const CanonPath & pathInStore);
