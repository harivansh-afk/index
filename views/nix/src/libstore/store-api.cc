...
std::pair<CanonPath, std::optional<std::string>> Store::getStoreObjectFingerprint(const CanonPath & pathInStore)
{
    /* The store as a whole is not a store object, so it has no content
       hash to answer with. */
    if (pathInStore.isRoot())
        return {pathInStore, std::nullopt};

    std::optional<std::pair<StorePath, CanonPath>> split;
    try {
        split = toStorePath(storeDir + pathInStore.abs());
    } catch (BadStorePath &) {
        /* Something under the store directory that is not shaped like a
           store object at all, so it has no hash part to look up:
           `.links`, or a path a caller composed by hand. */
        return {pathInStore, std::nullopt};
    }
    auto & [storePath, rest] = *split;

    /* Only a REGISTERED store object is immutable. A whole-store
       accessor built with `requireValidPath = false` will also serve
       unregistered content that merely sits under the store directory
       and is named like a store object: a build's `<drv>.chroot`
       sandbox root, which `chroot-derivation-builder.cc` puts beside
       the outputs on purpose, a half-written path, a `.check` tree.
       Nothing declares any of those stable and no hash is recorded for
       them, so decline to cache. */
    if (!isValidPath(storePath))
        return {pathInStore, std::nullopt};

    /* Sound because a valid store object is immutable: the store never
       rewrites one in place, so the NAR hash recorded when it was
       registered keeps describing the bytes on disk for as long as it
       stays valid. That is exactly what a fingerprint has to promise,
       namely that one fingerprint can only ever stand for one set of
       bytes.

       The recorded hash is the identifier, NOT the store path. An
       input-addressed output path is a function of its derivation and
       not of its content, so a non-reproducible build re-run after a
       garbage collection can leave different bytes at the same path;
       keying on the path alone would then answer with the hash from
       before the collection. Keying on the NAR hash cannot: different
       bytes give a different hash and therefore a different cache key.

       `path:` is the same fingerprint namespace `PathInputScheme`
       already uses for a store path (`fetchers/path.cc`), and it means
       the same thing here, so the two share cache entries for the same
       tree instead of computing it twice. */
    return {rest, fmt("path:%s", queryPathInfo(storePath)->narHash.to_string(HashFormat::SRI, true))};
}
...
StorePathSet Store::queryValidPaths(const StorePathSet & paths, SubstituteFlag maybeSubstitute)
{
    ...
    /* Create the pool after everything the work items reference, so its
       destructor joins the workers before any of that is destroyed. */
    ThreadPool pool;

    for (auto & path : paths)
        if (!pool.tryEnqueue(std::bind(doQuery, path)))
            break;
