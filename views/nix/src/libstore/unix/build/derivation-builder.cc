...
public:
    ...
    ~DerivationBuilderImpl() override
    {
        /* A joinable thread here would abort the process, so this is the last
           guarantee that the relay never outlives the build it belongs to.
           Everything that ends a build calls `stopLogRelay` before this,
           having first killed the builder, so by now it is a no-op. */
        stopLogRelay();
    }

    std::optional<pid_t> getPid() override
    {
        auto value = static_cast<pid_t>(pid);
    ...
    void cleanupOnDestruction() noexcept
    {
        ...
        /* After the child, so that the relay has had the chance to forward
           everything the builder said before it died. */
        stopLogRelay();
        try {
            stopDaemon();
        } catch (...) {
...
protected:
     ...
     *
     * Reads from @p from rather than @ref builderOut, because not every caller
     * runs late enough for @ref builderOut to exist: `startLogRelay()` is what
     * creates it, and the clone helper failure path in
     * `ChrootLinuxDerivationBuilder::startChild()` reports its error before
     * that. Such a caller passes @ref builderOutMaster instead.
     */
    void processSandboxSetupMessages(Descriptor from);

    std::string drainRemainingLog() override;

    /**
     * Master side of the builder's pseudoterminal. Read only by
     * @ref logRelayThread, which is joined before this is closed.
     */
    AutoCloseFD builderOutMaster;

    /**
     * Copies @ref builderOutMaster into @ref builderOut for as long as the
     * builder has anything to say. See `startLogRelay`.
     */
    std::thread logRelayThread;

    /**
     * How @ref logRelayThread is told to stop when the build is abandoned
     * rather than finished. Only the write side lives in this process.
     */
    Pipe logRelayQuit;

    /**
     * Start @ref logRelayThread, and point @ref builderOut at the pipe it
     * fills. Called once the builder is running.
     */
    void startLogRelay();

    /**
     * Stop @ref logRelayThread and release the pseudoterminal master.
     * Idempotent, and safe to call while the builder is still alive.
     */
    void stopLogRelay() noexcept;
...
/**
 * Open the destination named by `write-through-store`, reusing the store for
 * subsequent builds. Opening one costs a connection or a cache handshake, which
 * would otherwise be paid once per derivation.
 */
static ref<Store> getWriteThroughStore(const std::string & uri)
{
    static std::mutex mutex;
    static std::map<std::string, ref<Store>> stores;

    std::lock_guard<std::mutex> lock(mutex);
    if (auto i = stores.find(uri); i != stores.end())
        return i->second;
    return stores.emplace(uri, openStore(uri)).first->second;
}

/**
 * Copy the outputs just registered by a local build to the `write-through-store`
 * destination.
 *
 * This runs before the goal reports success so that durability precedes
 * visibility: anything that observes the build as done can rely on the outputs
 * being on the destination, and so can treat this store as a cache it may prune.
 * A failure here is a build failure, because outputs that exist in one place
 * only have not been made durable.
 */
static void publishOutputs(LocalStore & store, const StorePath & drvPath, const SingleDrvOutputs & builtOutputs)
{
    auto & uri = settings.writeThroughStore.get();
    if (uri.empty())
        return;

    StorePathSet paths;
    for (auto & [outputName, realisation] : builtOutputs)
        paths.insert(realisation.outPath);

    /* Only the outputs, not their closure. Every reference of theirs is already
       durable somewhere: one that was built here went through this same step
       when it was built, and one that was substituted still exists in the store
       it came from. The destination can therefore be missing a reference, and a
       consumer of it needs the same substituters this machine had. */
    try {
        copyPaths(store, *getWriteThroughStore(uri), paths, NoRepair, NoCheckSigs, NoSubstitute);
    } catch (Error & e) {
        throw BuildError(
            BuildResult::Failure::TransientFailure,
            "the outputs of '%s' were built but not published: write-through to the store '%s' failed: %s",
...
SingleDrvOutputs DerivationBuilderImpl::unprepareBuild()
{
    ...
    /* The builder is gone, so nothing more can arrive: stop the relay and
    ...
    /* After the build's own state is torn down, so that a publication failure
       does not also leak the build directory. */
    publishOutputs(store, drvPath, builtOutputs);
...
std::optional<Descriptor> DerivationBuilderImpl::startBuild()
{
    ...
    for (auto & [outputName, status] : initialOutputs) {
        ...
        if (!status.known) {
            /* The scratch path is a fallback path: deterministic in the
               derivation and output name, and by construction never a
               registered store path. A build killed after its builder
               created $out (possible on non-chroot stores, where the
               builder writes at the scratch store path directly) leaves
               that path behind as an invalid, build-user-owned orphan;
               the next build of the same derivation computes the same
               scratch path and its builder cannot write $out over it
               (indexable-inc/index#2247). We hold this derivation's
               output locks here, so an on-disk path that is not valid
               cannot be anything but such an orphan, and nothing can
               register it concurrently: clear it and proceed. Never
               touch a valid path. */
            auto scratchPathAbs = store.printStorePath(scratchPath);
            if (pathExists(scratchPathAbs) && !store.isValidPath(scratchPath)) {
                printInfo(
                    "clearing invalid orphan '%s' left by a killed build of '%s'",
                    scratchPathAbs,
                    store.printStorePath(drvPath));
                deletePath(scratchPathAbs);
            }
            continue;
        }
    ...
    builderOutMaster = posix_openpt(O_RDWR | O_NOCTTY);
    if (!builderOutMaster)
        throw SysError("opening pseudoterminal master");

    std::string slaveName = getPtsName(builderOutMaster.get());
    ...
    if (buildUser) {
        ...
        if (grantpt(builderOutMaster.get()))
            throw SysError("granting access to pseudoterminal slave");
    }
#endif

    if (unlockpt(builderOutMaster.get()))
    ...
    processSandboxSetupMessages(builderOut.get());
...
void DerivationBuilderImpl::openSlave()
{
    ...
    AutoCloseFD slave = open(slaveName.c_str(), O_RDWR | O_NOCTTY);
    if (!slave)
        throw SysError("opening pseudoterminal slave");

    // Put the pt into raw mode to prevent \n -> \r\n translation.
    struct termios term;
    if (tcgetattr(slave.get(), &term))
        throw SysError("getting pseudoterminal attributes");

    cfmakeraw(&term);

    if (tcsetattr(slave.get(), TCSANOW, &term))
        throw SysError("putting pseudoterminal into raw mode");

    if (dup2(slave.get(), STDERR_FILENO) == -1)
...
/**
 * Copy everything the builder says from the pseudoterminal master to a pipe,
 * until the last slave fd closes or @p quit says to stop.
 *
 * Nothing here allocates, and nothing here blocks anywhere but in the two
 * polls, both of which watch @p quit. A scheduler that has stopped reading
 * fills the pipe, which stops this thread, which fills the pseudoterminal,
 * which blocks the builder: the backpressure chain the scheduler had directly
 * on the master, one buffer longer.
 */
void relayBuilderOut(Descriptor master, Descriptor sink, Descriptor quit)
{
    ...
    while (true) {
        struct pollfd wait[2] = {
            ...
            {.fd = quit, .events = POLLIN, .revents = 0},
        };
        if (::poll(wait, 2, -1) == -1) {
            if (errno == EINTR)
                continue;
        ...
        /* Anything at all on `quit` -- a byte, a hangup -- is the end of this
           thread's job, and it is checked first so that teardown cannot be
           held up by a builder that is still talking. */
        if (wait[1].revents)
            return;
        if (!wait[0].revents)
            continue;

        auto rd = ::read(master, buf.data(), buf.size());
        if (rd == -1 && errno == EINTR)
            continue;
        /* 0 or EIO both mean the last slave fd is gone, which is the end of
           the builder's output; see `MuxablePipePollState::iterate`. Any other
           error is not something this thread can report, and closing `sink`
           tells the scheduler the same thing the builder's death would. */
        if (rd <= 0)
            return;

        for (size_t done = 0; done < (size_t) rd;) {
            struct pollfd ready[2] = {
                {.fd = sink, .events = POLLOUT, .revents = 0},
                {.fd = quit, .events = POLLIN, .revents = 0},
            };
            if (::poll(ready, 2, -1) == -1) {
                if (errno == EINTR)
            ...
            if (ready[1].revents)
                return;
            auto wr = ::write(sink, buf.data() + done, (size_t) rd - done);
            if (wr == -1) {
                /* `sink` is non-blocking so that a full pipe comes back here,
                   where `quit` is honoured, rather than parking this thread
                   somewhere teardown cannot reach it. */
                if (errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK)
                    continue;
                /* EPIPE: nobody is reading the log any more. */
                return;
            }
            done += (size_t) wr;
        }
...
void DerivationBuilderImpl::startLogRelay()
{
    /* The scheduler drains every awake goal before it reaches `waitForInput()`,
       so it can go seconds between reads of any one builder's output, and
       Darwin destroys whatever the pseudoterminal still holds ~0.6s after the
       last slave fd closes: XNU's ptsclose() gives the master that long to
       drain (t_timeout = 60 ticks) and ttyclose() then calls ttyflush(FREAD |
       FWRITE) on the queue regardless. A builder that writes its diagnostic
       and exits at once therefore used to lose its whole log whenever this
       process was busy starting other children, which is the normal state of a
       parallel build.

       So the read of the master does not wait on the scheduler: this thread
       keeps the pseudoterminal empty and hands the bytes to a pipe, which
       holds them for as long as it takes anyone to ask. The scheduler polls
       that pipe, and since nothing in this process holds a slave fd, the
       master still reaches EOF when the builder closes the last one -- the
       thread turns that into EOF on the pipe, so the end of the log means what
       it always did. */
    Pipe relay;
    relay.create();
    logRelayQuit.create();

    if (fcntl(relay.writeSide.get(), F_SETFL, fcntl(relay.writeSide.get(), F_GETFL) | O_NONBLOCK) == -1)
        throw SysError("making the builder's log pipe non-blocking");

    auto master = builderOutMaster.get();
    /* The write end of the relay is the thread's own, so the pipe reaches EOF
       exactly when the thread is done with it. Everything else it touches is
       borrowed and outlives it, `stopLogRelay` being the only thing that
       closes those, after the join. In particular the read end of the quit
       pipe stays here: a thread that closed it on the way out would turn the
       wake-up write below into SIGPIPE, which kills any program that has not
       chosen to ignore that signal. */
    auto sink = relay.writeSide.release();
    auto quit = logRelayQuit.readSide.get();

    builderOut = std::move(relay.readSide);

    logRelayThread = std::thread([master, sink, quit] {
        relayBuilderOut(master, sink, quit);
        close(sink);
    });
}

void DerivationBuilderImpl::stopLogRelay() noexcept
{
    if (!logRelayThread.joinable())
        return;

    /* A readable byte rather than a hangup, which some pollers report as
       POLLHUP and others as POLLIN. */
    char stop = 0;
    [[maybe_unused]] auto ignored = ::write(logRelayQuit.writeSide.get(), &stop, 1);

    logRelayThread.join();

    /* Nothing reads these now, so they are ours to close. */
    logRelayQuit.readSide.close();
    logRelayQuit.writeSide.close();
    builderOutMaster.close();
}

void DerivationBuilderImpl::processSandboxSetupMessages(Descriptor from)
{
    std::vector<std::string> msgs;
    while (true) {
        std::string msg = [&]() {
            try {
                /* One byte at a time, so that nothing of the builder's own
                   output is read past the end of the setup messages and lost
                   to the buffer of a source nobody reads again. */
                return readLine(from);
            } catch (Error & e) {
                /* `Pid::wait()` opens with `assert(pid != -1)`, and there are paths
                   here with no builder child to wait for: the clone helper can
                   fail before `startChild()` has a pid to report. Aborting the
                   process while reporting a build failure is strictly worse than
                   reporting it without an exit status, so the status is only
                ...
                if (pid != -1)
                    e.addTrace(
                        ...
                        store.printStorePath(drvPath),
                        statusToString(pid.wait()),
                        concatStringsSep("|", msgs));
                else
                    e.addTrace(
                        {},
                        "while waiting for the build environment for '%s' to initialize (no builder process was started, previous messages: %s)",
                        store.printStorePath(drvPath),
                        concatStringsSep("|", msgs));
                throw;
            }
...
std::string DerivationBuilderImpl::drainRemainingLog()
{
    if (!builderOut)
        return {};
    ...
    std::string data;
    while (true) {
        struct pollfd fd = {.fd = builderOut.get(), .events = POLLIN, .revents = 0};
        auto res = ::poll(&fd, 1, 0);
        if (res == -1) {
            if (errno == EINTR)
                continue;
            throw SysError("draining the builder's output");
        }
        /* Whatever the relay thread has not handed over yet is lost, which is
           only ever the tail of a build that is being abandoned anyway. */
        if (res == 0)
            break;
        char buf[4096];
        auto rd = ::read(builderOut.get(), buf, sizeof(buf));
        if (rd > 0) {
            data.append(buf, (size_t) rd);
            continue;
        }
        if (rd == -1 && errno == EINTR)
            continue;
        /* 0 or an error: the relay thread is gone and so is its pipe. */
        break;
    }
    return data;
}
    ...
    for (auto & outputName : sortedOutputNames) {
        ...
        if (store.toRealPath(newInfo.path) != actualPath) {
            if (buildMode == bmRepair) {
                ...
                /* The final path is already registered, so there is nothing
                   to move: keep what is registered and drop our copy.

                   For a content-addressed output this is the ordinary case.
                   The path name is derived from the content, so validity of
                   that name is proof that something else produced bytes
                   identical to ours, and picking either is the same thing.

                   For an input-addressed output the name encodes only this
                   derivation's inputs, so validity says nothing about
                   content, and reaching here means the path became valid
                   after `DerivationBuildingGoal` took this derivation's
                   output locks and re-checked validity. That is impossible
                   when a store answers `isValidPath` from the same place its
                   lock files live, which is the assumption this code used to
                   assert. It does not hold for a local-overlay store:
                   validity there is also answered from the lower store,
                   whose writers this store's lock files do not cover, and
                   the manual asks only that the lower store *grow* while it
                   is mounted -- so a supported configuration can validate
                   the path mid-build. Aborting the process over that is not
                   a defensible response to a documented use.

                   Prefer the registered path info over ours. The registered
                   object is what is on disk and what any referrer or GC root
                   already points at, and its NAR hash describes those bytes;
                   ours would describe them only if the derivation is
                   deterministic. Registering our hash against someone else's
                   content would put a lie in the database that later surfaces
                   as a corrupt path or a rejected copy, far from its cause.
                   A difference here is real non-determinism, so say so rather
                   than silently discard the evidence. */
                if (!newInfo.ca) {
                    ValidPathInfo registered(*store.queryPathInfo(newInfo.path));
                    if (registered.narHash != newInfo.narHash)
                        warn(
                            "derivation '%s' may not be deterministic: output '%s' became valid "
                            "while this build was running, and the registered path differs from "
                            "what this build produced; keeping the registered path",
                            store.printStorePath(drvPath),
                            store.printStorePath(newInfo.path));
                    newInfo = std::move(registered);
                }
