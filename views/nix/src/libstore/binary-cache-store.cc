    ...
    Source & narSource, RepairFlag repair, CheckSigsFlag checkSigs, fun<ValidPathInfo(HashResult)> mkInfo)
{
    ...
    HashSink narHashSink{HashAlgorithm::SHA256};
    {
        ...
        auto compressionSink = makeCompressionSink(
            config.compression,
            teeSinkCompressed,
            // A nonzero thread count implies parallel compression on its
            // own: see parallelCompressionThreads's doc comment.
            config.parallelCompression || config.parallelCompressionThreads > 0,
    ...
    if (config.writeDebugInfo) {
        ...
        if (auto st = narAccessor->maybeLstat(buildIdDir); st && st->type == SourceAccessor::tDirectory) {
            ...
            /* Create the pool after everything the work items reference, so
               its destructor joins the workers before any of that is
               destroyed. */
            ThreadPool threadPool(25);

            /* Once a work item throws, the pool refuses new items; stop
               feeding it and let process() rethrow that exception. */
            ...
            for (auto & [s1, _type] : narAccessor->readDirectory(buildIdDir)) {
                ...
                for (auto & [s2, _type] : narAccessor->readDirectory(dir)) {
                    ...
                    if (!threadPool.tryEnqueue(std::bind(doFile, std::string(debugPath.rel()), key, target))) {
                        poolShutDown = true;
                        break;
                    }
                }
