...
struct GitRepoImpl : GitRepo, std::enable_shared_from_this<GitRepoImpl>
{
    ...
    std::optional<Hash> getArchiveCompatibleTree(const Hash & rev) override
    {
        ...
        auto callback = [](const char * root, const git_tree_entry * entry, void * payload) -> int {
            ...
            if (git_tree_entry_type(entry) == GIT_OBJECT_BLOB
                ...
                if (contents.find("export-ignore") != contents.npos || contents.find("export-subst") != contents.npos) {
                    state.compatible = false;
                    return -1;
                }
            }
    ...
    /* Make git materialise the objects reachable from HEAD without walking
       history, which is the set the status walk reads. Returns whether git
       succeeded; a repository missing objects for any other reason fails here
       too, and the caller reports that rather than walking again. */
    bool materialiseHeadObjects()
    {
        /* A lazy fetch can reach the network, and this runs under a fetcher
           that has no terminal to prompt on, so refuse the prompt rather than
           block on it: an error here is recoverable, a hang is not. */
        auto env = getEnvOs();
        env[OS_STR("GIT_TERMINAL_PROMPT")] = OS_STR("0");
        ...
        return runProgram(std::move(opts)).first == 0;
    }
    ...
    WorkdirInfo getWorkdirInfo() override
    {
        ...
        if (git_status_foreach_ext(*this, &options, &statusCallbackTrampoline, &statusCallback)) {
            ...
            if (git_status_foreach_ext(*this, &options, &statusCallbackTrampoline, &statusCallback))
                throw GitError(
                    "getting working directory status for %s, which is missing objects git could not supply either",
                    PathFmt(path));
        }
...
struct GitSourceAccessor : SourceAccessor
{
    ...
    bool pathExists(const CanonPath & path) override
    {
        ...
        return entry && !(state->options.omitGitlinks && git_tree_entry_filemode(entry) == GIT_FILEMODE_COMMIT);
    }
    ...
    std::optional<Stat> maybeLstat(const CanonPath & path) override
    {
        ...
        if (state->options.omitGitlinks && mode == GIT_FILEMODE_COMMIT)
            return std::nullopt;
            ...
            overloaded{
                [&](Tree tree) {
                    ...
                    for (size_t n = 0; n < count; ++n) {
                        ...
                        if (state->options.omitGitlinks && git_tree_entry_filemode(entry) == GIT_FILEMODE_COMMIT)
                            continue;
                        // FIXME: add to cache
...
static Sync<std::map<std::filesystem::path, GitRepo::WorkdirInfo>> & workdirInfoCache()
{
    static Sync<std::map<std::filesystem::path, GitRepo::WorkdirInfo>> cache;
    return cache;
}

GitRepo::WorkdirInfo GitRepo::getCachedWorkdirInfo(const std::filesystem::path & path)
{
    auto & _cache = workdirInfoCache();
    {
...
void GitRepo::clearCachedWorkdirInfo()
{
    workdirInfoCache().lock()->clear();
}
