...
{
    {
        ...
        if (useRustEval) {
            ...
            if (!evalOnly || parseOnly)
                throw Error("rust-eval unimplemented: nix-instantiate without --eval");
        }

        if (readStdin) {
            if (useRustEval)
                throw Error("rust-eval unimplemented: reading from stdin");
        ...
        for (auto & i : files) {
            if (useRustEval) {
                if (fromArgs)
                    rustEvalPrint(*state, i, absPath("."), attrPaths, outputKind, strict);
                else {
