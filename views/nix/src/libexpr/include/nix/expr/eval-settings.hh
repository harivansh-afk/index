...
struct EvalSettings : Config
{
    ...
    Setting<std::string> evalBackend{
        ...
        R"(
          ...
          interpreter) or `rust` (the in-tree Rust evaluator, requires the
          `rust-eval` experimental feature). The Rust backend fails with
          `rust-eval unimplemented: <construct>` on anything it does not
          cover yet; it never falls back silently.
        )"};
