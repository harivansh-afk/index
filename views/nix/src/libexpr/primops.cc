...
/**
 * The prefix `lib/modules.nix` puts on the error context of every option
 * value it computes:
 *
 *     value = addErrorContext "while evaluating the option `${showOption loc}':" value;
 *
 * Matching on it is how an option value is recognised without the evaluator
 * knowing anything about the module system, and without patching nixpkgs.
 */
static constexpr std::string_view optionContextPrefix = "while evaluating the option `";

static void prim_addErrorContext(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    /* Recognising an option value means reading the context message, which a
       run without instrumentation never does on the success path. That extra
       work is why `read-set-track-options` is off by default. */
    std::string optionKey;
    if (state.readSetTracker && state.settings.readSetTrackOptions) [[unlikely]] {
        try {
            NixStringContext context;
            auto message = state.coerceToString(pos, *args[0], context, "", false, false).toOwned();
            if (message.starts_with(optionContextPrefix)) {
                optionKey = message.substr(optionContextPrefix.size());
                /* The message ends "`<option>':", so drop the two characters
                   that close the quote rather than carrying them in the key. */
                if (optionKey.ends_with("':"))
                    optionKey.resize(optionKey.size() - 2);
            }
        } catch (Error &) {
            /* A context message that cannot be forced is not an option, and
               is not this instrumentation's business to report. */
        }
    }

    ReadSetFrame frame(
        optionKey.empty() ? nullptr : state.readSetTracker.get(), TrackedEntryKind::option, [&] { return optionKey; });
...
{
    /* The tracked derivation boundary. Pushed before the name is known, so
       that the forcing of the derivation's own attributes is inside it, and
       renamed below once there is a name to use. */
    ReadSetFrame frame(state.readSetTracker.get(), TrackedEntryKind::derivation, [] { return "<unnamed>"; });
    ...
    frame.setKey([&] { return std::string(drvName); });

    /* The retained-graph splice. When a previous request for the same
       installable left a graph behind and this entry validates clean
       against it, the fully forced result is returned without forcing
       the derivation attributes, which is the work this boundary holds.
       Identity across requests is the derivation name plus how many
       times that name has been seen this request; that is an assumption
       about demand order, not a guarantee, which is one of the reasons
       this only runs under an explicit eval-persistent flag. */
    if (state.retainedPrev && state.readSetTracker) [[unlikely]] {
        auto resolvedEntry = state.retainedPrev->resolveDerivation(
            drvName, state.readSetTracker->parentOldId(), *state.readSetTracker);
        if (resolvedEntry.entry) {
            /* Even a dirty counterpart anchors its children: the entries this
               one demands while recomputing resolve among the old edges. */
            state.readSetTracker->setCurrentOldId(int64_t(resolvedEntry.entry->id));
            if (!resolvedEntry.dirty) {
                v = *resolvedEntry.entry->value;
                state.readSetTracker->noteDerivationProduced(resolvedEntry.entry->produced);
                state.readSetTracker->noteDerivationResult(v);
                return;
            }
        }
    }
...
static void derivationStrictInternal(EvalState & state, std::string_view drvName, const Bindings * attrs, Value & v)
{
    ...
    /* The derivations whose outputs this one consumes, which is Nix's own
       record of what flowed in and costs nothing extra to read. This is the
       edge the phase 1 measurement needed and did not have: of the
       derivations one commit moves, one reads the edited bytes and the rest
       move because their input derivations moved. */
    if (state.readSetTracker) [[unlikely]]
        for (auto & inputDrv : drv.inputDrvs.map)
            state.readSetTracker->noteDerivationDemand(state.store->printStorePath(inputDrv.first));

    /* The source paths this derivation consumes, which `inputDrvs` does not
       cover and which no derivation output stands in for. A derivation that
       embeds a path from the flake tree takes the tree's materialised store
       path as an input source, so recording only input derivations left that
       whole class unrooted: on the measured ix edit it left `claude-skills-farm`
       looking valid, and with it the four derivations below it, because the
       only thing that moved was this list. A store path is a hash of its
       contents, so the name changing is the content changing and comparing two
       traces needs nothing else. */
    if (state.readSetTracker) [[unlikely]]
        for (auto & inputSrc : drv.inputSrcs)
            state.readSetTracker->recordStoreQuery(state.store->printStorePath(inputSrc));
    ...
    /* Named by its store path, because that is how a consumer of this
       derivation names it: the path is what lands in the string context that
       `noteDerivationDemand` above reads back. */
    if (state.readSetTracker) [[unlikely]]
        state.readSetTracker->noteDerivationProduced(drvPathS);
    ...
    /* Retain the result while it is still just strings, so a later
       request can answer this same derivation without redoing the attr
       forcing above. No-op unless the tracker was built to retain. */
    if (state.readSetTracker) [[unlikely]]
        state.readSetTracker->noteDerivationResult(v);
}
...
static void prim_storePath(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    if (state.readSetTracker) [[unlikely]]
        state.readSetTracker->recordStoreQuery(state.store->printStorePath(storePath));
...
static void prim_readFile(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    if (state.readSetTracker) [[unlikely]]
        state.readSetTracker->provRegisterFileValue(path, v);
}
...
static void prim_fromJSON(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ProvScope provScope(state.readSetTracker.get());
    if (state.readSetTracker) [[unlikely]]
        state.readSetTracker->provConsume(*args[0]);
...
{
    /* The produced path names a file whose bytes came from the contents
       argument, so the path string inherits the contents' provenance: a
       consumer embedding the path embeds those bytes at build time. */
    ProvScope provScope(state.readSetTracker.get());
    NixStringContext context;
...
void prim_getAttr(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    state.forceValue(*i->value, pos);
    if (state.readSetTracker) [[unlikely]]
        state.readSetTracker->provSelect(*args[1], *i->value);
    v = *i->value;
}
...
/**
 * Record that a position's line or column was observed, which is what makes
 * an edit that shifts lines a change to a real input. Recording the observed
 * value, not just the file, so that a shift changes the recorded input.
 */
static void recordPositionObservation(EvalState & state, PosIdx posIdx)
{
    if (state.readSetTracker && state.settings.readSetTrackPositions) [[unlikely]]
        state.readSetTracker->recordPosition(state.positions[posIdx], true);
}

static struct LazyPosAccessors
{
    PrimOp primop_lineOfPos{.arity = 1, .impl = [](EvalState & state, PosIdx pos, Value ** args, Value & v) {
                                auto posIdx = PosIdx(args[0]->integer().value);
                                recordPositionObservation(state, posIdx);
                                v.mkInt(state.positions[posIdx].line);
                            }};
    PrimOp primop_columnOfPos{.arity = 1, .impl = [](EvalState & state, PosIdx pos, Value ** args, Value & v) {
                                  auto posIdx = PosIdx(args[0]->integer().value);
                                  recordPositionObservation(state, posIdx);
                                  v.mkInt(state.positions[posIdx].column);
                              }};
...
static void prim_removeAttrs(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    if (state.readSetTracker) [[unlikely]]
        state.readSetTracker->provSelect(*args[0], v);
}
...
static void prim_intersectAttrs(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    v.mkAttrs(attrs.alreadySorted());
    /* The result's values come from the right operand. */
    if (state.readSetTracker) [[unlikely]]
        state.readSetTracker->provSelect(*args[1], v);
}
...
static void prim_catAttrs(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    for (auto v2 : args[1]->listView()) {
        ...
        if (auto i = v2->attrs()->get(attrName)) {
            if (state.readSetTracker) [[unlikely]]
                state.readSetTracker->provSelect(*v2, *i->value);
            res[found++] = i->value;
        }
    }
...
static void prim_mapAttrs(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    v.mkAttrs(attrs.alreadySorted());
    /* The result's thunks close over the input's values; registering the
       result container lets the select hook attribute their eventual
       forcing back to the input's producer. */
    if (state.readSetTracker) [[unlikely]]
        state.readSetTracker->provSelect(*args[1], v);
}
...
void prim_match(EvalState & state, const PosIdx pos, Value ** args, Value & v)
{
    ...
    try {
        ...
        if (auto * tracker = state.readSetTracker.get()) [[unlikely]] {
            ProvScope provScope(tracker);
            tracker->provConsume(*args[1]);
            provScope.finish(v);
        }
