...
use ix_kernel::{
    ...
    dispatch::{on_perform, Outcome, PerformCtx, Performed},
    Domain, EffectLock, KernelConfig, KernelError, MemoTable, ObjId, Policy, RefreshPolicy,
    Provenance, Result,
};
...
impl<C: Cas> Kernel<C> {
    ...
    fn perform<F>(&mut self, domain: Domain, policy: &Policy, req: &[u8], run: F) -> Result<Performed>
    where
...
fn run_program<C: Cas>(kernel: &mut Kernel<C>, ledger: &Ledger) -> Result<Vec<Outcome>> {
    let read = kernel.perform(domain("read"), &Policy::Keyed, &request("build.conf"), || {
