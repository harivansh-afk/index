...
impl DirCas {
    ...
    pub fn open(root: impl Into<PathBuf>) -> Result<Self> {
        ...
        fs::create_dir_all(&root)
            .map_err(|source| KernelError::io(format!("creating store {}", root.display()), source))?;
        Ok(Self { root })
    }
...
impl Cas for DirCas {
    fn put(&self, bytes: &[u8]) -> Result<ObjId> {
        ...
        let mut file = fs::File::create(&temp_path)
            .map_err(|source| KernelError::io(format!("creating {}", temp_path.display()), source))?;
        let written = file
            .write_all(bytes)
            .and_then(|()| file.flush())
    ...
    fn get(&self, id: ObjId) -> Result<Option<Vec<u8>>> {
        ...
        match fs::read(&path) {
            ...
            Err(source) => Err(KernelError::io(format!("reading {}", path.display()), source)),
        }
    }
